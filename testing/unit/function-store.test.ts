...
import { FunctionStore } from '../../packages/function-store/src';
import { save } from '../../packages/function-store/src';
import { update } from '../../packages/function-store/src';

function createTempDir() {
    return fs.mkdtempSync(path.join(os.tmpdir(), 'elit-function-store-'));
}

describe('function store helpers preserve typed declarations', () => {
    ...
    it('executes ES module JavaScript database files without requiring CommonJS packaging', async () => {
        ...
        try {
            ...
            const db = new FunctionStore({ dir, language: 'js' });
    ...
    it('strips TypeScript syntax from database modules without a bundled esbuild dependency', async () => {
        ...
        try {
            ...
            const db = new FunctionStore({ dir, language: 'ts' });
            const result = await db.execute(`
                import { users } from '@db/users';
                console.log(users[0].name);
            `);
