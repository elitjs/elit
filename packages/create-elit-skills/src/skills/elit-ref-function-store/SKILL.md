...
### Server route using FunctionStore
...
- Writing to the `databases/*.ts` files from client code. The database is server-only — keep `@elitjs/function-store` out of client bundles.
