# elit/function-store — Code-Stored Collections

`FunctionStore` loads `.ts` (or `.js`) files from a directory; each file defines a collection by exporting query functions. The class executes the file's code in a VM context.
...
```ts
// src/server.ts
const db = new FunctionStore({ dir: resolve(process.cwd(), 'databases'), language: 'ts' });
```
...
Create multiple `FunctionStore` instances with different `dir`s:

```ts
const tenantA = new FunctionStore({ dir: './databases/tenant-a' });
const tenantB = new FunctionStore({ dir: './databases/tenant-b' });
```
...
- **`FunctionStore` per-request** — connection-leak / perf cliff. Initialize once per process.
